...
use aurion_core::{command::EditCommand, directory::Directory, mode::Mode};
use aurion_proxy::plugin::{download_volt, volt_icon, wasi::find_all_volts};
use aurion_rpc::{
...
use crate::{
    ...
    config::{Aurion IDEConfig, color::Aurion IDEColor},
    db::Aurion IDEDb,
    editor::EditorData,
...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
            let url = format!(
                "https://plugins.Aurion IDE.dev/api/v1/plugins/{}/{}/latest",
            ...
            std::thread::spawn(move || {
                let info: Option<VoltInfo> = aurion_proxy::get_url(url, None)
                    .ok()
    ...
    fn load_icon(volt: &VoltInfo) -> Result<VoltIcon> {
        let url = format!(
            "https://plugins.Aurion IDE.dev/api/v1/plugins/{}/{}/{}/icon?id={}",
        ...
        let content = match cache_content {
            ...
            None => {
                let resp = aurion_proxy::get_url(&url, None)?;
    ...
    fn download_readme(
        ...
        let url = format!(
            "https://plugins.Aurion IDE.dev/api/v1/plugins/{}/{}/{}/readme",
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        let url = format!(
            "https://plugins.Aurion IDE.dev/api/v1/plugins?q={query}&offset={offset}"
        );
        let plugins: VoltsInfo = aurion_proxy::get_url(url, None)?.json()?;
        Ok(plugins)
    }
            ...
            label(move || control(local_version_info.clone()))
                .style(move |s| {
                    ...
                    s.margin_left(10)
                        ...
                        .color(
                            config
                                .color(Aurion IDEColor::Aurion IDE_BUTTON_PRIMARY_FOREGROUND),
                        )
                        .background(
                            config
                                .color(Aurion IDEColor::Aurion IDE_BUTTON_PRIMARY_BACKGROUND),
                        )
                        .hover(|s| {
                            s.cursor(CursorStyle::Pointer).background(
                                config
                                    .color(
                                        Aurion IDEColor::Aurion IDE_BUTTON_PRIMARY_BACKGROUND,
                                    )
                                    .multiply_alpha(0.8),
                            )
                        })
                        ...
                        .disabled(|s| {
                            s.background(config.color(Aurion IDEColor::EDITOR_DIM))
                        })
                        .selectable(false)
                })
                                ...
                                stack((
                                    ...
                                    web_link(
                                        ...
                                        move || {
                                            config
                                                .get()
                                                .color(Aurion IDEColor::EDITOR_LINK)
                                        },
                                        internal_command,
                                    ),
                                ))
                                    ...
                                    MarkdownContent::Separator => {
                                        container(empty().style(move |s| {
                                            s.width_full()
                                                ...
                                                .background(
                                                    config.get().color(
                                                        Aurion IDEColor::Aurion IDE_BORDER,
                                                    ),
                                                )
                                        }))
                                    }
