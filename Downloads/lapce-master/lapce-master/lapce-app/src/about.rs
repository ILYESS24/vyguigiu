...
use aurion_core::{command::FocusCommand, meta::VERSION, mode::Mode};

use crate::{
    command::{CommandExecuted, CommandKind},
    config::color::Aurion IDEColor,
...
impl AboutUri {
    const Aurion IDE: &'static str = "https://Aurion IDE.dev";
    const GITHUB: &'static str = "https://github.com/Aurion IDE/Aurion IDE";
    const MATRIX: &'static str = "https://matrix.to/#/#Aurion IDE-editor:matrix.org";
        ...
        stack((
            svg(move || (config.get()).logo_svg()).style(move |s| {
                s.size(logo_size, logo_size)
                    .color(config.get().color(Aurion IDEColor::EDITOR_FOREGROUND))
            }),
            label(|| "Aurion IDE".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
                    .color(config.get().color(Aurion IDEColor::EDITOR_FOREGROUND))
            }),
            ...
            web_link(
                || "Website".to_string(),
                || AboutUri::Aurion IDE.to_string(),
                move || config.get().color(Aurion IDEColor::EDITOR_LINK),
                internal_command,
            )
...
fn exclusive_popup<V: View + 'static>(
    ...
    .style(move |s| {
        ...
        .background(
            config
                .get()
                .color(Aurion IDEColor::Aurion IDE_DROPDOWN_SHADOW)
                .multiply_alpha(0.5),
        )
    })
}
