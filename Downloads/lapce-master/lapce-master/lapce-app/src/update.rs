...
use aurion_core::{directory::Directory, meta};
...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/Aurion IDE/Aurion IDE/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/Aurion IDE/Aurion IDE/releases/latest",
    };

    let resp = aurion_proxy::get_url(url, Some("Aurion IDE"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = match std::env::consts::OS {
        "macos" => "Aurion IDE-macos.dmg",
        "linux" => match std::env::consts::ARCH {
            "aarch64" => "Aurion IDE-linux-arm64.tar.gz",
            "x86_64" => "Aurion IDE-linux-amd64.tar.gz",
            _ => return Err(anyhow!("arch not supported")),
        },
        #[cfg(feature = "portable")]
        "windows" => "Aurion IDE-windows-portable.zip",
        #[cfg(not(feature = "portable"))]
        "windows" => "Aurion IDE-windows.msi",
    ...
    for asset in &release.assets {
        if asset.name == name {
            let mut resp = aurion_proxy::get_url(&asset.browser_download_url, None)?;
            if !resp.status().is_success() {
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::remove_dir_all(dest.join("Aurion IDE.app"))?;
    fs_extra::copy_items(
        &[info.mount_point.join("Aurion IDE.app")],
        dest,
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::copy(parent.join("Aurion IDE").join("Aurion IDE"), process_path)?;
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("Aurion IDE.exe.bak"))?;
    std::fs::copy(parent.join("Aurion IDE.exe"), process_path)?;
...
pub fn restart(path: &Path) -> Result<()> {
    ...
    let Aurion IDE_exe = std::env::current_exe()
        .map_err(|err| anyhow!("can't get path to exe").context(err))?;
    let Aurion IDE_exe = Aurion IDE_exe
        .to_str()
        .ok_or_else(|| anyhow!("can't convert exe path to str"))?;

    std::process::Command::new("cmd")
        .raw_arg(format!(
            r#"/C taskkill /PID {process_id} & msiexec /i "{path}" /qb & start "" "{Aurion IDE_exe}""#,
        ))
        .creation_flags(DETACHED_PROCESS)
        .spawn()?;
    Ok(())
...
pub fn cleanup() {
    ...
    if let Ok(process_path) = std::env::current_exe() {
        if let Some(dst_parent) = process_path.parent() {
            if let Err(err) = std::fs::remove_file(dst_parent.join("Aurion IDE.exe.bak"))
            {
                tracing::error!("{:?}", err);
            }
        }
