...
mod tests {
    ...
    use crate::{config::Aurion IDEConfig, workspace::Aurion IDEWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = Aurion IDEWorkspace::default();

        let config = Aurion IDEConfig::merge_config(&workspace, None, None);
        let mut Aurion IDE_config: Aurion IDEConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        Aurion IDE_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // Aurion IDE_config.available_icon_themes = Some(vec![]);
        Aurion IDE_config.core.color_theme = "test".to_string();
