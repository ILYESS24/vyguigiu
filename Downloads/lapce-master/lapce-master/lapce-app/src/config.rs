...
use aurion_core::directory::Directory;
use aurion_proxy::plugin::wasi::find_all_volts;
use aurion_rpc::plugin::VoltID;
...
static DEFAULT_CONFIG: Lazy<config::Config> = Lazy::new(Aurion IDEConfig::default_config);
static DEFAULT_Aurion IDE_CONFIG: Lazy<Aurion IDEConfig> =
    Lazy::new(Aurion IDEConfig::default_Aurion IDE_config);
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        Aurion IDEConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl Aurion IDEConfig {
    pub fn load(
        ...
        let mut Aurion IDE_config: Aurion IDEConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
                error!("Failed to deserialize configuration file: {error}");
                DEFAULT_Aurion IDE_CONFIG.clone()
            }
        };

        Aurion IDE_config.available_color_themes =
            Self::load_color_themes(disabled_volts, extra_plugin_paths);
        Aurion IDE_config.available_icon_themes =
            Self::load_icon_themes(disabled_volts, extra_plugin_paths);
        Aurion IDE_config.resolve_theme(workspace);

        Aurion IDE_config.color_theme_list = Aurion IDE_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        Aurion IDE_config.color_theme_list.sort();

        Aurion IDE_config.icon_theme_list = Aurion IDE_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        Aurion IDE_config.icon_theme_list.sort();

        Aurion IDE_config.wrap_style_list = im::vector![
        ...
        Aurion IDE_config.terminal.get_indexed_colors();
        ...
        match workspace.kind {
            Aurion IDEWorkspaceType::Local => {
                if let Some(path) = workspace.path.as_ref() {
                    let path = path.join("./.Aurion IDE/settings.toml");
                    config = config::Config::builder()
                        .add_source(config.clone())
                        .add_source(
            ...
            Aurion IDEWorkspaceType::RemoteSSH(_) => {}
            #[cfg(windows)]
            Aurion IDEWorkspaceType::RemoteWSL(_) => {}
        }
    ...
    fn default_Aurion IDE_config() -> Aurion IDEConfig {
        let mut default_Aurion IDE_config: Aurion IDEConfig =
        ...
        default_Aurion IDE_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_Aurion IDE_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_Aurion IDE_config.resolve_colors(None);
        default_Aurion IDE_config
    }

    fn resolve_theme(&mut self, workspace: &Aurion IDEWorkspace) {
        let default_Aurion IDE_config = DEFAULT_Aurion IDE_CONFIG.clone();
        ...
        .try_deserialize::<Aurion IDEConfig>()
        {
            self.core = new.core;
            self.ui = new.ui;
        ...
        self.resolve_colors(Some(&default_Aurion IDE_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &Aurion IDEWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &Aurion IDEWorkspace, theme: &str) {
        self.core.icon_theme = theme.to_string();
        self.resolve_theme(workspace);
    }

    pub fn set_modal(&mut self, _workspace: &Aurion IDEWorkspace, modal: bool) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&Aurion IDEConfig>) {
        ...
        let fg = self.color(Aurion IDEColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(Aurion IDEColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(Aurion IDEColor::Aurion IDE_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
            ...
            (
                self.ui_svg(Aurion IDEIcons::FILE),
                Some(self.color(Aurion IDEColor::Aurion IDE_ICON_ACTIVE)),
            )
        }
    }
    ...
    pub fn symbol_svg(&self, kind: &SymbolKind) -> Option<String> {
        let kind_str = match *kind {
            SymbolKind::ARRAY => Aurion IDEIcons::SYMBOL_KIND_ARRAY,
            SymbolKind::BOOLEAN => Aurion IDEIcons::SYMBOL_KIND_BOOLEAN,
            SymbolKind::CLASS => Aurion IDEIcons::SYMBOL_KIND_CLASS,
            SymbolKind::CONSTANT => Aurion IDEIcons::SYMBOL_KIND_CONSTANT,
            SymbolKind::ENUM_MEMBER => Aurion IDEIcons::SYMBOL_KIND_ENUM_MEMBER,
            SymbolKind::ENUM => Aurion IDEIcons::SYMBOL_KIND_ENUM,
            SymbolKind::EVENT => Aurion IDEIcons::SYMBOL_KIND_EVENT,
            SymbolKind::FIELD => Aurion IDEIcons::SYMBOL_KIND_FIELD,
            SymbolKind::FILE => Aurion IDEIcons::SYMBOL_KIND_FILE,
            SymbolKind::INTERFACE => Aurion IDEIcons::SYMBOL_KIND_INTERFACE,
            SymbolKind::KEY => Aurion IDEIcons::SYMBOL_KIND_KEY,
            SymbolKind::FUNCTION => Aurion IDEIcons::SYMBOL_KIND_FUNCTION,
            SymbolKind::METHOD => Aurion IDEIcons::SYMBOL_KIND_METHOD,
            SymbolKind::OBJECT => Aurion IDEIcons::SYMBOL_KIND_OBJECT,
            SymbolKind::NAMESPACE => Aurion IDEIcons::SYMBOL_KIND_NAMESPACE,
            SymbolKind::NUMBER => Aurion IDEIcons::SYMBOL_KIND_NUMBER,
            SymbolKind::OPERATOR => Aurion IDEIcons::SYMBOL_KIND_OPERATOR,
            SymbolKind::TYPE_PARAMETER => Aurion IDEIcons::SYMBOL_KIND_TYPE_PARAMETER,
            SymbolKind::PROPERTY => Aurion IDEIcons::SYMBOL_KIND_PROPERTY,
            SymbolKind::STRING => Aurion IDEIcons::SYMBOL_KIND_STRING,
            SymbolKind::STRUCT => Aurion IDEIcons::SYMBOL_KIND_STRUCT,
            SymbolKind::VARIABLE => Aurion IDEIcons::SYMBOL_KIND_VARIABLE,
            _ => return None,
        };
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (Aurion IDEColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (Aurion IDEColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (Aurion IDEColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (Aurion IDEColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (Aurion IDEColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (Aurion IDEColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (Aurion IDEColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (Aurion IDEColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (Aurion IDEColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (Aurion IDEColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (Aurion IDEColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (Aurion IDEColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (Aurion IDEColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (Aurion IDEColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (Aurion IDEColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (Aurion IDEColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (Aurion IDEColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (Aurion IDEColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (Aurion IDEColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (Aurion IDEColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (Aurion IDEColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (Aurion IDEColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (Aurion IDEColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (Aurion IDEColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (Aurion IDEColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (Aurion IDEColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (Aurion IDEColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (Aurion IDEColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (Aurion IDEColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
