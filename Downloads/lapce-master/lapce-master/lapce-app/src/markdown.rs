...
use aurion_core::{language::Aurion IDELanguage, syntax::Syntax};
use Aurion IDE_xi_rope::Rope;
use lsp_types::MarkedString;
use pulldown_cmark::{CodeBlockKind, CowStr, Event, Options, Parser, Tag};
use smallvec::SmallVec;

use crate::config::{Aurion IDEConfig, color::Aurion IDEColor};
                        ...
                        Tag::CodeBlock(kind) => {
                            let language =
                                if let CodeBlockKind::Fenced(language) = kind {
                                    md_language_to_Aurion IDE_language(language)
                                } else {
                                    None
                                };
            ...
            Event::Html(text) => {
                attr_list.add_span(
                    ...
                    default_attrs
                        ...
                        .color(config.color(Aurion IDEColor::MARKDOWN_BLOCKQUOTE)),
                );
                current_text.push_str(&text);
                pos += text.len();
                builder_dirty = true;
...
fn attribute_for_tag<'a>(
    ...
    match tag {
        ...
        Tag::Link {
            ...
            // TODO: Link support
            Some(default_attrs.color(config.color(Aurion IDEColor::EDITOR_LINK)))
        }
...
fn md_language_to_Aurion IDE_language(lang: &str) -> Option<Aurion IDELanguage> {
    // TODO: There are many other names commonly used that should be supported
    Aurion IDELanguage::from_name(lang)
}
...
pub fn highlight_as_code(
    ...
    default_attrs: Attrs,
    language: Option<Aurion IDELanguage>,
    text: &str,
    start_offset: usize,
    config: &Aurion IDEConfig,
) {
