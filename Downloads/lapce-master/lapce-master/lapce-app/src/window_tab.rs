...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, Aurion IDECommand,
        Aurion IDEWorkbenchCommand, WindowCommand,
    },
    completion::{CompletionData, CompletionStatus},
    config::Aurion IDEConfig,
    db::Aurion IDEDb,
    debug::{DapData, Aurion IDEBreakpoint, RunDebugMode, RunDebugProcess},
    doc::DocContent,
    ...
    workspace::{Aurion IDEWorkspace, Aurion IDEWorkspaceType, WorkspaceInfo},
};
...
pub struct CommonData {
    pub workspace: Arc<Aurion IDEWorkspace>,
    pub scope: Scope,
    ...
    pub Aurion IDE_command: Listener<Aurion IDECommand>,
    pub workbench_command: Listener<Aurion IDEWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<Aurion IDEConfig>>,
    pub proxy_status: RwSignal<Option<ProxyStatus>>,
    pub mouse_hover_timer: RwSignal<TimerToken>,
    pub breakpoints: RwSignal<BTreeMap<PathBuf, BTreeMap<usize, Aurion IDEBreakpoint>>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<Aurion IDEConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = Aurion IDEConfig::load(
            &workspace,
            &all_disabled_volts,
            &window_common.extra_plugin_paths,
        );
        let Aurion IDE_command = Listener::new_empty(cx);
                    ...
                    .map(|(path, breakpoints)| {
                        (
                            ...
                            breakpoints
                                .into_iter()
                                .map(|b| (b.line, b))
                                .collect::<BTreeMap<usize, Aurion IDEBreakpoint>>(),
                        )
                    })
                    .collect(),
            );
...

        {
            ...
            window_tab_data.common.Aurion IDE_command.listen(move |cmd| {
                window_tab_data.run_Aurion IDE_command(cmd);
            });
        }
        ...
        if !change_plugins.is_empty() {
            ...
            if config.core.auto_reload_plugin {
                let mut plugin_metas: HashMap<
                    String,
                    aurion_rpc::plugin::VoltMetadata,
                > = self
                    .plugin
                    .installed
                    .get_untracked()
    ...
    pub fn run_Aurion IDE_command(&self, cmd: Aurion IDECommand) {
        match cmd.kind {
    ...
    pub fn run_workbench_command(
        ...
        use Aurion IDEWorkbenchCommand::*;
        match cmd {
            // ==== Modal ====
            EnableModal => {
                    ...
                    open_file(options, move |file| {
                        if let Some(mut file) = file {
                            let workspace = Aurion IDEWorkspace {
                                kind: Aurion IDEWorkspaceType::Local,
            ...
            InstallToPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        ...
                        arguments: vec![String::from("-e"), format!(r#"do shell script "ln -sf '{}' /usr/local/bin/Aurion IDE" with administrator privileges"#, std::env::args().next().unwrap())],
                    }
                )
            }
            #[cfg(target_os = "macos")]
            UninstallFromPATH => {
                self.common.internal_command.send(
                    InternalCommand::ExecuteProcess {
                        program: String::from("osascript"),
                        arguments: vec![String::from("-e"), String::from(r#"do shell script "rm /usr/local/bin/Aurion IDE" with administrator privileges"#)],
                    }
                )
            }
            JumpLocationForward => {
