...
use aurion_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::Aurion IDELanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use aurion_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, Aurion IDECommand, WindowCommand,
    },
    db::Aurion IDEDb,
    ...
    workspace::{Aurion IDEWorkspace, Aurion IDEWorkspaceType, SshHost},
};
...
impl PaletteData {
        ...
        let items = workspaces
            ...
            .filter_map(|w| {
                ...
                let filter_text = match &w.kind {
                    Aurion IDEWorkspaceType::Local => text,
                    Aurion IDEWorkspaceType::RemoteSSH(remote) => {
                        format!("[{remote}] {text}")
                    }
                    #[cfg(windows)]
                    Aurion IDEWorkspaceType::RemoteWSL(remote) => {
                        format!("[{remote}] {text}")
                    }
                };
                Some(PaletteItem {
    ...
    fn get_ssh_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let Aurion IDEWorkspaceType::RemoteSSH(host) = &workspace.kind {
    ...
    fn get_wsl_hosts(&self) {
        ...
        for workspace in workspaces.iter() {
            if let Aurion IDEWorkspaceType::RemoteWSL(host) = &workspace.kind {
    ...
    fn set_run_configs(&self, content: String) {
        ...
        if configs.is_none() {
            if let Some(path) = self.workspace.path.as_ref() {
                let path = path.join(".Aurion IDE").join("run.toml");
                self.common
                    .internal_command
    ...
    fn get_run_configs(&self) {
        if let Some(workspace) = self.common.workspace.path.as_deref() {
            let run_toml = workspace.join(".Aurion IDE").join("run.toml");
    ...
    fn get_languages(&self) {
        let langs = Aurion IDELanguage::languages();
        let items = langs
            .iter()
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    let cmd = Aurion IDECommand {
                        kind: CommandKind::Workbench(cmd.clone()),
                        data: None,
                    };

                    self.common.Aurion IDE_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.Aurion IDE_command.send(cmd.clone());
                }
                    ...
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: Aurion IDEWorkspace {
                                kind: Aurion IDEWorkspaceType::RemoteSSH(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::WslHost { host } => {
                    self.common.window_common.window_command.send(
                        WindowCommand::SetWorkspace {
                            workspace: Aurion IDEWorkspace {
                                kind: Aurion IDEWorkspaceType::RemoteWSL(host.clone()),
                                path: None,
                                last_open: 0,
                            },
                        },
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match Aurion IDELanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .Aurion IDE_command
                        .send(crate::command::Aurion IDECommand {
                        kind: CommandKind::Workbench(
                            crate::command::Aurion IDEWorkbenchCommand::CheckoutReference,
                        ),
            ...
            self.common.window_common.window_command.send(
                WindowCommand::SetWorkspace {
                    workspace: Aurion IDEWorkspace {
                        kind: Aurion IDEWorkspaceType::RemoteSSH(ssh),
                        path: None,
                        last_open: 0,
                    },
                },
