...
use aurion_core::meta;
use aurion_rpc::proxy::ProxyStatus;

use crate::{
    app::{clickable_icon, not_clickable_icon, tooltip_label, window_menu},
    command::{Aurion IDECommand, Aurion IDEWorkbenchCommand, WindowCommand},
    config::{Aurion IDEConfig, color::Aurion IDEColor, icon::Aurion IDEIcons},
    listener::Listener,
...
fn left(
    ...
    stack((
        ...
        container(svg(move || config.get().ui_svg(Aurion IDEIcons::LOGO)).style(
            move |s| {
        ...
        not_clickable_icon(
            || Aurion IDEIcons::MENU,
            || false,
            || false,
            || "Menu",
            config,
        )
        .popout_menu(move || window_menu(Aurion IDE_command, workbench_command))
        .style(move |s| {
            s.margin_left(4.0)
                .margin_right(6.0)
        ...
        tooltip_label(
            config,
            container(svg(move || config.get().ui_svg(Aurion IDEIcons::REMOTE)).style(
                move |s| {
                    let config = config.get();
                    ...
                    s.size(size, size).color(if is_local {
                        ...
                        match proxy_status.get() {
                            Some(_) => Color::WHITE,
                            None => config.color(Aurion IDEColor::Aurion IDE_ICON_ACTIVE),
                        }
                    })
                },
            )),
        ...
        .popout_menu(move || {
            ...
            let mut menu = Menu::new("").entry(
                MenuItem::new("Connect to SSH Host").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::ConnectSshHost);
                }),
            );
            if !is_local
                ...
                })
            {
                menu = menu.entry(MenuItem::new("Disconnect remote").action(
                    move || {
                        workbench_command
                            .send(Aurion IDEWorkbenchCommand::DisconnectRemote);
                    },
                ));
            }
            #[cfg(windows)]
            {
                menu = menu.entry(MenuItem::new("Connect to WSL Host").action(
                    move || {
                        workbench_command
                            .send(Aurion IDEWorkbenchCommand::ConnectWslHost);
                    },
                ));
            }
            menu
            ...
            let color = if is_local {
                ...
                match proxy_status.get() {
                    Some(ProxyStatus::Connected) => {
                        config.color(Aurion IDEColor::Aurion IDE_REMOTE_CONNECTED)
                    }
                    Some(ProxyStatus::Connecting) => {
                        config.color(Aurion IDEColor::Aurion IDE_REMOTE_CONNECTING)
                    }
                    Some(ProxyStatus::Disconnected) => {
                        config.color(Aurion IDEColor::Aurion IDE_REMOTE_DISCONNECTED)
                    }
...
fn middle(
    ...
    let jump_backward = move || {
        clickable_icon(
            || Aurion IDEIcons::LOCATION_BACKWARD,
            move || {
                workbench_command.send(Aurion IDEWorkbenchCommand::JumpLocationBackward);
            },
            || false,
    ...
    let jump_forward = move || {
        clickable_icon(
            || Aurion IDEIcons::LOCATION_FORWARD,
            move || {
                workbench_command.send(Aurion IDEWorkbenchCommand::JumpLocationForward);
            },
            || false,
        ...
        .popout_menu(move || {
            Menu::new("")
                ...
                .entry(MenuItem::new("Open Recent Workspace").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::PaletteWorkspace);
                }))
        })
    };
    ...
    stack((
        ...
        container(
            stack((
                svg(move || config.get().ui_svg(Aurion IDEIcons::SEARCH)).style(
                    move |s| {
                        let config = config.get();
        ...
        .on_click_stop(move |_| {
            if workspace.clone().path.is_some() {
                workbench_command.send(Aurion IDEWorkbenchCommand::PaletteHelpAndFile);
            } else {
                workbench_command.send(Aurion IDEWorkbenchCommand::PaletteWorkspace);
            }
        })
        ...
        stack((
            clickable_icon(
                || Aurion IDEIcons::START,
                move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::PaletteRunAndDebug)
                },
                || false,
                || false,
                || "Run and Debug",
    ...
    stack((
        ...
        stack((
            ...
            .popout_menu(move || {
                Menu::new("")
                    .entry(MenuItem::new("Command Palette").action(move || {
                        workbench_command.send(Aurion IDEWorkbenchCommand::PaletteCommand)
                    }))
                    .separator()
                    .entry(MenuItem::new("Open Settings").action(move || {
                        workbench_command.send(Aurion IDEWorkbenchCommand::OpenSettings)
                    }))
                    .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                        move || {
                            workbench_command
                                .send(Aurion IDEWorkbenchCommand::OpenKeyboardShortcuts)
                        },
                    ))
                    .entry(MenuItem::new("Open Theme Color Settings").action(
                        move || {
                            workbench_command
                                .send(Aurion IDEWorkbenchCommand::OpenThemeColorSettings)
                        },
                    ))
                    .separator()
                    .entry(if let Some(v) = latest_version.get_untracked() {
                        if update_in_progress.get_untracked() {
                            ...
                            MenuItem::new(format!("Restart to update ({v})")).action(
                                move || {
                                    workbench_command
                                        .send(Aurion IDEWorkbenchCommand::RestartToUpdate)
                                },
                            )
                        }
                    } else {
            ...
            container(label(|| "1".to_string()).style(move |s| {
                let config = config.get();
                s.font_size(10.0)
                    .color(config.color(Aurion IDEColor::EDITOR_BACKGROUND))
                    .border_radius(100.0)
                    .margin_left(5.0)
                    .margin_top(10.0)
                    .background(config.color(Aurion IDEColor::EDITOR_CARET))
            }))
            .style(move |s| {
...
pub fn title(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    let Aurion IDE_command = window_tab_data.common.Aurion IDE_command;
...
pub fn window_controls_view(
    ...
    stack((
        clickable_icon(
            || Aurion IDEIcons::WINDOW_MINIMIZE,
            || {
        ...
        clickable_icon(
            move || {
                if window_maximized.get() {
                    Aurion IDEIcons::WINDOW_RESTORE
                } else {
                    Aurion IDEIcons::WINDOW_MAXIMIZE
                }
            },
            move || {
