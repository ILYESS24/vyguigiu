...
use crate::{
    ...
    workspace::{Aurion IDEWorkspace, WorkspaceInfo},
};
...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(Aurion IDEWorkspace, WorkspaceInfo),
    RecentWorkspace(Aurion IDEWorkspace),
    Doc(DocInfo),
    DisabledVolts(Vec<VoltID>),
    WorkspaceDisabledVolts(Arc<Aurion IDEWorkspace>, Vec<VoltID>),
    PanelOrder(PanelOrder),
}

#[derive(Clone)]
pub struct Aurion IDEDb {
    folder: PathBuf,
...
impl Aurion IDEDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<Aurion IDEWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<Aurion IDEWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &Aurion IDEWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: Aurion IDEWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &Aurion IDEWorkspace) -> String {
