...
use aurion_core::selection::Selection;
use aurion_rpc::{
...
fn file_node_text_color(
    ...
    let color = match diff {
        ...
        Some(FileDiffKind::Added) => Aurion IDEColor::SOURCE_CONTROL_ADDED,
        Some(FileDiffKind::Deleted) => Aurion IDEColor::SOURCE_CONTROL_REMOVED,
        None => Aurion IDEColor::PANEL_FOREGROUND,
    };
                        ...
                        move |s| {
                            s.height(ui_line_height.get())
                                .color(
                                    config
                                        .get()
                                        .color(Aurion IDEColor::PANEL_FOREGROUND_DIM),
                                )
                                .selectable(false)
                        },
                    ),
            ...
            stack((
                ...
                label(move || err.clone()).style(move |s| {
                    ...
                    let editor_background_color =
                        config.color(Aurion IDEColor::PANEL_CURRENT_BACKGROUND);
                    let error_background_color =
                        config.color(Aurion IDEColor::ERROR_LENS_ERROR_BACKGROUND);
                    ...
                    s.position(Position::Absolute)
                        ...
                        .width_full()
                        .color(config.color(Aurion IDEColor::ERROR_LENS_ERROR_FOREGROUND))
                ...
                let view = stack((
                    svg(move || {
                        ...
                        let svg_str = match open {
                            true => Aurion IDEIcons::ITEM_OPENED,
                            false => Aurion IDEIcons::ITEM_CLOSED,
                        };
                    ...
                    }),
                    {
                        ...
                        svg(move || {
                            ...
                            if is_dir {
                                let svg_str = match open {
                                    true => Aurion IDEIcons::DIRECTORY_OPENED,
                                    false => Aurion IDEIcons::DIRECTORY_CLOSED,
                                };
                                config.ui_svg(svg_str)
                            } else if let Some(path) = kind.path() {
                                config.file_svg(path).0
                            } else {
                                config.ui_svg(Aurion IDEIcons::FILE)
                            }
                        })
                        .style(move |s| {
                            let config = config.get();
                        ...
                        s.padding_right(15.0)
                            ...
                            .apply_if(
                                ...
                                |x| {
                                    x.background(
                                        config.get().color(
                                            Aurion IDEColor::PANEL_CURRENT_BACKGROUND,
                                        ),
                                    )
                                },
                            )
