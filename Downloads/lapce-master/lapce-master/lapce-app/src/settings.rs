...
use aurion_core::{buffer::rope_text::RopeText, mode::Mode};
use aurion_rpc::plugin::VoltID;
use Aurion IDE_xi_rope::Rope;
use serde::Serialize;
...
use crate::{
    ...
    config::{
        DropdownInfo, Aurion IDEConfig, color::Aurion IDEColor, core::CoreConfig,
        editor::EditorConfig, icon::Aurion IDEIcons, terminal::TerminalConfig,
        ui::UIConfig,
    },
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> aurion_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::Aurion IDECommand,
        _count: Option<usize>,
        _mods: Modifiers,
        ...
        .style(move |s| {
            ...
            s.padding_horiz(20.0)
                ...
                .apply_if(kind == current_kind.get(), |s| {
                    s.background(config.color(Aurion IDEColor::PANEL_CURRENT_BACKGROUND))
                })
                .hover(|s| {
            ...
            if let Some(is_ticked) = is_ticked {
                ...
                create_effect(move |last| {
                    ...
                    if let Ok(value) = serde::Serialize::serialize(
                        ...
                        Aurion IDEConfig::update_file(&kind, &field, value);
                    }
                });
                                ...
                                if let Some(timer) = timer.try_get_untracked() {
                                    if timer == token {
                                        ...
                                        if default != Some(&value) {
                                            ...
                                            Aurion IDEConfig::reset_setting(
                                                &format!("color-theme.{kind}"),
                                                &field,
                                            );
                                        }
                        ...
                        text("Reset")
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(Aurion IDEColor::Aurion IDE_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(Aurion IDEIcons::CLOSE)
                } else {
                    config.get().ui_svg(Aurion IDEIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
