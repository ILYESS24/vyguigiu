...
impl ExpandedRunDebug {
    pub fn expand(
        ...
        let mut program = if program == "${Aurion IDE}" {
            std::env::current_exe()
                .map_err(|e| {
                    anyhow!(
                        "Failed to get current exe for ${{Aurion IDE}} run and debug: {e}"
                    )
                })?
                .to_str()
                .ok_or_else(|| anyhow!("Failed to convert ${{Aurion IDE}} path to str"))?
                .to_string()
        } else {
            program
        };
