...
use aurion_core::mode::{Mode, VisualMode};
use lsp_types::{DiagnosticSeverity, ProgressToken};

use crate::{
    app::clickable_icon,
    command::Aurion IDEWorkbenchCommand,
...
pub fn status(
    ...
    _config: ReadSignal<Arc<Aurion IDEConfig>>,
) -> impl View {
        ...
        stack((
            ...
            .style(move |s| {
                ...
                let (bg, fg) = match mode.get() {
                    Mode::Normal => (
                        Aurion IDEColor::STATUS_MODAL_NORMAL_BACKGROUND,
                        Aurion IDEColor::STATUS_MODAL_NORMAL_FOREGROUND,
                    ),
                    Mode::Insert => (
                        Aurion IDEColor::STATUS_MODAL_INSERT_BACKGROUND,
                        Aurion IDEColor::STATUS_MODAL_INSERT_FOREGROUND,
                    ),
                    Mode::Visual(_) => (
                        Aurion IDEColor::STATUS_MODAL_VISUAL_BACKGROUND,
                        Aurion IDEColor::STATUS_MODAL_VISUAL_FOREGROUND,
                    ),
                    Mode::Terminal => (
                        Aurion IDEColor::STATUS_MODAL_TERMINAL_BACKGROUND,
                        Aurion IDEColor::STATUS_MODAL_TERMINAL_FOREGROUND,
                    ),
                };
            ...
            stack((
                svg(move || config.get().ui_svg(Aurion IDEIcons::SCM)).style(move |s| {
                    let config = config.get();
                ...
                label(branch).style(move |s| {
                    s.margin_left(10.0)
                        .color(config.get().color(Aurion IDEColor::STATUS_FOREGROUND))
                        .selectable(false)
                }),
            ))
            .style(move |s| {
                ...
                .hover(|s| {
                    s.cursor(CursorStyle::Pointer).background(
                        config.get().color(Aurion IDEColor::PANEL_HOVERED_BACKGROUND),
                    )
                })
            })
            ...
            .on_event(
                ...
                move |_| {
                    if pointer_down.get() {
                        workbench_command
                            .send(Aurion IDEWorkbenchCommand::PaletteSCMReferences);
                    }
            ...
            ),
            {
                ...
                stack((
                    svg(move || config.get().ui_svg(Aurion IDEIcons::ERROR)).style(
                        move |s| {
                            let config = config.get();
                        ...
                        move |s| {
                            s.margin_left(5.0)
                                .color(
                                    config
                                        .get()
                                        .color(Aurion IDEColor::STATUS_FOREGROUND),
                                )
                                .selectable(false)
                        },
                    ),
                    svg(move || config.get().ui_svg(Aurion IDEIcons::WARNING)).style(
                        move |s| {
                            let config = config.get();
                            let size = config.ui.icon_size() as f32;
                            s.size(size, size)
        ...
        stack((
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            Aurion IDEIcons::SIDEBAR_LEFT
                        } else {
                            Aurion IDEIcons::SIDEBAR_LEFT_OFF
                        }
                    }
                };
                clickable_icon(
                ...
                let icon = {
                    ...
                    move || {
                        if panel.is_container_shown(
                            ...
                            Aurion IDEIcons::LAYOUT_PANEL
                        } else {
                            Aurion IDEIcons::LAYOUT_PANEL_OFF
                        }
                    }
                };
                clickable_icon(
            ...
            },
            {
                ...
                let icon = {
                    ...
                    move || {
                        ...
                        {
                            Aurion IDEIcons::SIDEBAR_RIGHT
                        } else {
                            Aurion IDEIcons::SIDEBAR_RIGHT_OFF
                        }
                    }
                };
                clickable_icon(
    ...
    .style(move |s| {
        ...
        s.border_top(1.0)
            ...
            .background(config.color(Aurion IDEColor::STATUS_BACKGROUND))
...
fn status_text<S: std::fmt::Display + 'static>(
    ...
    label(text).style(move |s| {
        ...
        s.display(display)
            ...
            .padding_horiz(10.0)
            .items_center()
            .color(config.color(Aurion IDEColor::STATUS_FOREGROUND))
            .hover(|s| {
