...
use crate::{
    ...
    command::{
        CommandKind, InternalCommand, Aurion IDECommand, Aurion IDEWorkbenchCommand,
        WindowCommand,
    },
    config::{
        Aurion IDEConfig, color::Aurion IDEColor, icon::Aurion IDEIcons, ui::TabSeparatorHeight,
    ...
    workspace::{Aurion IDEWorkspace, Aurion IDEWorkspaceType},
};
...
#[derive(Parser)]
#[clap(name = "Aurion IDE")]
#[clap(version=meta::VERSION)]
#[derive(Debug)]
struct Cli {
    /// Launch new window even if Aurion IDE is already running
    ...
    #[clap(value_parser = aurion_proxy::cli::parse_file_line_column)]
...
pub struct AppData {
    ...
    pub config: RwSignal<Arc<Aurion IDEConfig>>,
...
impl AppData {
    pub fn reload_config(&self) {
        let config =
            Aurion IDEConfig::load(&Aurion IDEWorkspace::default(), &[], &self.plugin_paths);
    ...
    pub fn new_window(&self, folder: Option<PathBuf>) {
        let config = self
            ...
            .or_else(|| {
                let db: Arc<Aurion IDEDb> = use_context().unwrap();
        ...
        let workspace = Aurion IDEWorkspace {
            path: folder,
            ..Default::default()
        };
    ...
    fn create_windows(
        ...
        if !dirs.is_empty() {
            ...
            for dir in dirs {
                ...
                {
                    Aurion IDEWorkspaceType::RemoteWSL(crate::workspace::WslHost {
                        host: String::new(),
                    })
                } else {
                    Aurion IDEWorkspaceType::Local
                };
                #[cfg(not(windows))]
                let workspace_type = Aurion IDEWorkspaceType::Local;
                ...
                let info = WindowInfo {
                    ...
                    tabs: TabsInfo {
                        active_tab: 0,
                        workspaces: vec![Aurion IDEWorkspace {
                            kind: workspace_type,
        ...
        if inital_windows == 0 {
            let mut info = db.get_window().unwrap_or_else(|_| WindowInfo {
                ...
                tabs: TabsInfo {
                    active_tab: 0,
                    workspaces: vec![Aurion IDEWorkspace::default()],
                },
            });
            info.tabs = TabsInfo {
                active_tab: 0,
    ...
    let view_fn = move |(i, layout_rect, child): (
        ...
        let child_view = {
                            ...
                            .apply_if(
                                !info.with(|info| info.is_pristine)
                                    && config.ui.tab_close_button
                                        == TabCloseButton::Off,
                                |s| s.color(config.color(Aurion IDEColor::Aurion IDE_WARN)),
                            )
                    })
            })
            .style(|s| s.padding(4.));
            ...
            let tab_close_button = clickable_icon(
                move || {
                    if hovered.get() || info.with(|info| info.is_pristine) {
                        Aurion IDEIcons::CLOSE
                    } else {
                        Aurion IDEIcons::UNSAVED
                    }
                },
                move || {
                    let editor_tab_id =
            ...
            .style(move |s| {
                s.items_center()
                    ...
                    .border_right(1.0)
                    .border_color(config.get().color(Aurion IDEColor::Aurion IDE_BORDER))
                    .padding_horiz(6.)
                    .gap(6.)
                    .grid()
            ...
            child_view
                ...
                .dragging_style(move |s| {
                    ...
                    s.border(1.0)
                        ...
                        .background(
                            config
                                .color(Aurion IDEColor::PANEL_BACKGROUND)
                                .multiply_alpha(0.7),
                        )
                        .border_color(config.color(Aurion IDEColor::Aurion IDE_BORDER))
                })
                ...
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.get().color(if is_focused() {
                            Aurion IDEColor::Aurion IDE_TAB_ACTIVE_UNDERLINE
                        } else {
                            Aurion IDEColor::Aurion IDE_TAB_INACTIVE_UNDERLINE
                        }))
                })
                .style(|s| {
                    s.absolute()
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .border_color(
                            config
                                .get()
                                .color(Aurion IDEColor::Aurion IDE_TAB_ACTIVE_UNDERLINE)
                                .multiply_alpha(0.5),
                        )
                })
        ...
        .style(move |s| {
            ...
            s.height_full()
                ...
                .hover(|s| s.background(config.color(Aurion IDEColor::HOVER_BACKGROUND)))
        })
    ...
    stack((
            ...
            (
                clip(empty().style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(config.color(Aurion IDEColor::PANEL_BACKGROUND))
                        .box_shadow_blur(3.0)
                        .box_shadow_color(
                            config.color(Aurion IDEColor::Aurion IDE_DROPDOWN_SHADOW),
                        )
                }))
                .style(move |s| {
                ...
                stack((
                    clickable_icon(
                        || Aurion IDEIcons::TAB_PREVIOUS,
                        move || {
                            workbench_command
                                .send(Aurion IDEWorkbenchCommand::PreviousEditorTab);
                        },
                        || false,
                        || false,
                        || "Previous Tab",
                    ...
                    clickable_icon(
                        || Aurion IDEIcons::TAB_NEXT,
                        move || {
                            workbench_command
                                .send(Aurion IDEWorkbenchCommand::NextEditorTab);
                        },
                        || false,
                        || false,
                        || "Next Tab",
        ...
        stack({
            ...
            (
                ...
                stack((
                    clickable_icon(
                        || Aurion IDEIcons::SPLIT_HORIZONTAL,
                        move || {
                            let editor_tab_id =
                                editor_tab.with_untracked(|t| t.editor_tab_id);
                    ...
                    stack((
                        ...
                        .style(move |s| {
                            s.height_full()
                                ...
                                .border_right(1.0)
                                .border_color(
                                    config.get().color(Aurion IDEColor::Aurion IDE_BORDER),
                                )
                        }),
                        container(
                            editor_container_view(
                ...
                .style(move |s| {
                    ...
                    s.absolute()
                        ...
                        .background(
                            config
                                .get()
                                .color(Aurion IDEColor::EDITOR_DRAG_DROP_BACKGROUND),
                        )
                })
...
fn split_resize_border(
    ...
    config: ReadSignal<Arc<Aurion IDEConfig>>,
) -> impl View {
            ...
            .style(move |s| {
                ...
                s.position(Position::Absolute)
                    ...
                    .apply_if(is_dragging, |s| {
                        ...
                        .background(config.get().color(Aurion IDEColor::EDITOR_CARET))
                    })
                    .hover(|s| {
                        s.cursor(match direction {
                            SplitDirection::Vertical => CursorStyle::ColResize,
    ...
    dyn_stack(
        ...
        move |(_, content)| {
            container(empty().style(move |s| {
                ...
                .background(config.get().color(Aurion IDEColor::Aurion IDE_BORDER))
            }))
            .style(move |s| {
...
fn main_split(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.border_color(config.color(Aurion IDEColor::Aurion IDE_BORDER))
            .background(config.color(Aurion IDEColor::EDITOR_BACKGROUND))
    ...
    let view = container(
        svg(move || config.get().ui_svg(icon()))
            .style(move |s| {
                ...
                let size = config.ui.icon_size() as f32;
                s.size(size, size)
                    .color(config.color(Aurion IDEColor::Aurion IDE_ICON_ACTIVE))
                    .disabled(|s| {
                        s.color(config.color(Aurion IDEColor::Aurion IDE_ICON_INACTIVE))
    ...
    .style(move |s| {
        ...
        s.padding(4.0)
            ...
            .apply_if(active_fn(), |s| {
                s.border_color(config.color(Aurion IDEColor::EDITOR_CARET))
            })
            .hover(|s| {
                s.cursor(CursorStyle::Pointer)
                    .background(config.color(Aurion IDEColor::PANEL_HOVERED_BACKGROUND))
            })
            .active(|s| {
                s.background(
                    config.color(Aurion IDEColor::PANEL_HOVERED_ACTIVE_BACKGROUND),
                )
            })
    });
...
fn tooltip_tip<V: View + 'static>(
    ...
    container(child).style(move |s| {
        ...
        s.padding_horiz(10.0)
            ...
            .color(config.color(Aurion IDEColor::TOOLTIP_FOREGROUND))
            .background(config.color(Aurion IDEColor::TOOLTIP_BACKGROUND))
            .border(1)
            .border_radius(6)
            .border_color(config.color(Aurion IDEColor::Aurion IDE_BORDER))
            .box_shadow_blur(3.0)
            .box_shadow_color(config.color(Aurion IDEColor::Aurion IDE_DROPDOWN_SHADOW))
            .margin_left(0.0)
            .margin_top(4.0)
    })
}
...
fn palette_item(
    workspace: Arc<Aurion IDEWorkspace>,
    i: usize,
    item: PaletteItem,
            ...
            container(
                stack((
                    ...
                    focus_text(
                        ...
                        move || config.get().color(Aurion IDEColor::EDITOR_FOCUS),
                    )
                    ...
                    .style(move |s| {
                        s.color(config.get().color(Aurion IDEColor::EDITOR_DIM))
                            .min_width(0.0)
                            .flex_grow(1.0)
                            .flex_basis(0.0)
                    }),
            ...
            container(
                stack((
                    svg(move || {
                        ...
                        config
                            .symbol_svg(&kind)
                            .unwrap_or_else(|| config.ui_svg(Aurion IDEIcons::FILE))
                    })
                    .style(move |s| {
                        let config = config.get();
                        let size = config.ui.icon_size() as f32;
                        s.min_width(size).size(size, size).margin_right(5.0).color(
                            config.symbol_color(&kind).unwrap_or_else(|| {
                                config.color(Aurion IDEColor::Aurion IDE_ICON_ACTIVE)
                            }),
                        )
                    }),
                    focus_text(
            ...
            container(
                stack((
                    svg(move || {
                        let config = config.get();
                        match mode {
                            RunDebugMode::Run => config.ui_svg(Aurion IDEIcons::START),
                            RunDebugMode::Debug => config.ui_svg(Aurion IDEIcons::DEBUG),
                        }
                    })
                    .style(move |s| {
                        let config = config.get();
    ...
    .style(move |s| {
        s.width_full()
            ...
            .apply_if(index.get() == i, |style| {
                style.background(
                    config.get().color(Aurion IDEColor::PALETTE_CURRENT_BACKGROUND),
                )
            })
    })
}
                    ...
                    .style(move |s| {
                        s.width_full().cursor(CursorStyle::Pointer).hover(|s| {
                            s.background(
                                config
                                    .get()
                                    .color(Aurion IDEColor::PANEL_HOVERED_BACKGROUND),
                            )
                        })
                    })
                },
    ...
    container(
        ...
        .style(move |s| {
            ...
            s.width(config.ui.palette_width() as f64)
                ...
                .border_color(config.color(Aurion IDEColor::Aurion IDE_BORDER))
                .flex_col()
                .background(config.color(Aurion IDEColor::PALETTE_BACKGROUND))
                .pointer_events_auto()
        }),
    )
    .style(move |s| {
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.get().ui_svg(Aurion IDEIcons::ERROR)
                    } else {
                        config.get().ui_svg(Aurion IDEIcons::WARNING)
                    }
                })
                .style(move |s| {
                    let config = config.get();
                    ...
                    let color = if let MessageType::ERROR = message.typ {
                        config.color(Aurion IDEColor::Aurion IDE_ERROR)
                    } else {
                        config.color(Aurion IDEColor::Aurion IDE_WARN)
                    };
                    s.min_width(size)
                        .size(size, size)
                        .margin_right(10.0)
...
fn completion(window_tab_data: Rc<WindowTabData>) -> impl View {
                    ...
                    .style(move |s| {
                        ...
                        s.padding_horiz(5.0)
                            ...
                            .apply_if(active.get() == i, |s| {
                                s.background(
                                    config.color(Aurion IDEColor::COMPLETION_CURRENT),
                                )
                            })
                            .hover(move |s| {
                                s.background(
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.color(Aurion IDEColor::COMPLETION_BACKGROUND))
...
fn code_action(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.get().color(Aurion IDEColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
fn rename(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        ...
        s.position(Position::Absolute)
            ...
            .margin_left(origin.x as f32)
            .margin_top(origin.y as f32)
            .background(config.get().color(Aurion IDEColor::PANEL_BACKGROUND))
            .border_radius(6.0)
            .padding(6.0)
    })
...
fn window_tab(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    .style(move |s| {
        let config = config.get();
        s.size_full()
            .color(config.color(Aurion IDEColor::EDITOR_FOREGROUND))
            .background(config.color(Aurion IDEColor::EDITOR_BACKGROUND))
            .font_size(config.ui.font_size() as f32)
            ...
            .class(floem::views::scroll::Handle, |s| {
                s.background(config.color(Aurion IDEColor::Aurion IDE_SCROLL_BAR))
            })
    })
...
fn workspace_title(workspace: &Aurion IDEWorkspace) -> Option<String> {
    let p = workspace.path.as_ref()?;
    let dir = p.file_name().unwrap_or(p.as_os_str()).to_string_lossy();
    Some(match &workspace.kind {
        Aurion IDEWorkspaceType::Local => format!("{dir}"),
        Aurion IDEWorkspaceType::RemoteSSH(remote) => format!("{dir} [{remote}]"),
        #[cfg(windows)]
        Aurion IDEWorkspaceType::RemoteWSL(remote) => format!("{dir} [{remote}]"),
    })
}
        ...
        stack((
            ...
            .dragging_style(move |s| {
                ...
                s.border(1.0)
                    ...
                    .color(
                        config
                            .color(Aurion IDEColor::EDITOR_FOREGROUND)
                            .multiply_alpha(0.7),
                    )
                    .background(
                        config
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    .height_full()
                    .border_color(
                        config.get().color(Aurion IDEColor::Aurion IDE_TAB_ACTIVE_UNDERLINE),
                    )
        ...
        container(clickable_icon(
            ...
            move || {
                window_data.run_window_command(WindowCommand::NewWorkspaceTab {
                    workspace: Aurion IDEWorkspace::default(),
                    end: true,
                });
            },
            || false,
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - Aurion IDE"),
            None => "Aurion IDE".to_string(),
        }
    })
    ...
    .window_menu(move || {
        ...
        if let Some((_, window_tab)) = window_tab {
            ...
            let Aurion IDE_command = window_tab.common.Aurion IDE_command;
            window_menu(Aurion IDE_command, workbench_command)
        } else {
            Menu::new("Aurion IDE")
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up Aurion IDE..");
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .stderr(stderr)
            .stdout(stdout)
            .env("Aurion IDE_LOG", "aurion_app::app=error,off")
            .spawn()
        {
            eprintln!("Failed to launch Aurion IDE: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    // in the existing Aurion IDE process
    if !cli.new {
        match get_socket() {
            Ok(socket) => {
    ...
    if let Err(err) = aurion_proxy::register_Aurion IDE_path() {
        tracing::error!("{:?}", err);
    }
    let db = match Aurion IDEDb::new() {
        Ok(db) => Arc::new(db),
        Err(e) => {
            #[cfg(windows)]
            logging::error_modal("Error", &format!("Failed to create Aurion IDEDb: {e}"));

            trace!(TraceLevel::ERROR, "Failed to create Aurion IDEDb: {e}");
            std::process::exit(1);
        }
    };
    let scope = Scope::new();
    ...
    if let Some(path) = Aurion IDEConfig::settings_file() {
    ...
    if let Some(path) = Aurion IDEConfig::keymaps_file() {
        if let Err(err) = watcher.watch(&path, notify::RecursiveMode::Recursive) {
            tracing::error!("{:?}", err);
        }
    }
    ...
    let config = Aurion IDEConfig::load(&Aurion IDEWorkspace::default(), &[], &plugin_paths);
    ...
    #[cfg(feature = "updater")]
    {
        ...
        std::thread::Builder::new()
            .name("Aurion IDEUpdater".to_owned())
            .spawn(move || {
                loop {
...
pub fn try_open_in_existing_process(
    ...
    aurion_rpc::stdio::write_msg(&mut socket, msg)?;
        ...
        std::thread::spawn(move || -> Result<()> {
            ...
            loop {
                let msg: Option<CoreMessage> =
                    aurion_rpc::stdio::read_msg(&mut reader)?;
...
pub fn window_menu(
    Aurion IDE_command: Listener<Aurion IDECommand>,
    workbench_command: Listener<Aurion IDEWorkbenchCommand>,
) -> Menu {
    Menu::new("Aurion IDE")
        .entry({
            let mut menu = Menu::new("Aurion IDE")
                .entry(MenuItem::new("About Aurion IDE").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::ShowAbout)
                }))
                .separator()
                .entry(
                    Menu::new("Settings...")
                        .entry(MenuItem::new("Open Settings").action(move || {
                            workbench_command
                                .send(Aurion IDEWorkbenchCommand::OpenSettings);
                        }))
                        .entry(MenuItem::new("Open Keyboard Shortcuts").action(
                            move || {
                                workbench_command.send(
                                    Aurion IDEWorkbenchCommand::OpenKeyboardShortcuts,
                                );
                            },
                        )),
                )
                .separator()
                .entry(MenuItem::new("Quit Aurion IDE").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide Aurion IDE"))
        ...
        .entry(
            Menu::new("File")
                .entry(MenuItem::new("New File").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::NewFile);
                }))
                .separator()
                .entry(MenuItem::new("Open").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::OpenFile);
                }))
                .entry(MenuItem::new("Open Folder").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::OpenFolder);
                }))
                .separator()
                .entry(MenuItem::new("Save").action(move || {
                    Aurion IDE_command.send(Aurion IDECommand {
                        kind: CommandKind::Focus(FocusCommand::Save),
                        data: None,
                    });
                }))
                .entry(MenuItem::new("Save All").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::SaveAll);
                }))
                .separator()
                .entry(MenuItem::new("Close Folder").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::CloseFolder);
                }))
                .entry(MenuItem::new("Close Window").action(move || {
                    workbench_command.send(Aurion IDEWorkbenchCommand::CloseWindow);
                })),
        )
        .entry(
            Menu::new("Edit")
