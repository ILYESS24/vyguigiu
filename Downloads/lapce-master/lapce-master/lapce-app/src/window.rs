...
pub struct TabsInfo {
    ...
    pub workspaces: Vec<Aurion IDEWorkspace>,
}
...
/// resized using your window manager. Normally Aurion IDE has only one
...
impl WindowData {
    pub fn new(
        ...
        let cx = Scope::new();
        let config =
            Aurion IDEConfig::load(&Aurion IDEWorkspace::default(), &[], &extra_plugin_paths);
        ...
        if window_tabs.with_untracked(|window_tabs| window_tabs.is_empty()) {
            let window_tab = Rc::new(WindowTabData::new(
                cx,
                Arc::new(Aurion IDEWorkspace::default()),
                common.clone(),
            ));
            window_tabs.update(|window_tabs| {
                window_tabs.push_back((cx.create_rw_signal(0), window_tab));
    ...
    pub fn info(&self) -> WindowInfo {
        let workspaces: Vec<Aurion IDEWorkspace> = self
            .window_tabs
            .get_untracked()
            .iter()
