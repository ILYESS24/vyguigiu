...
impl Directory {
    ...
    fn project_dirs() -> Option<ProjectDirs> {
        ProjectDirs::from("dev", "Aurion IDE", NAME)
    }

    /// Return path adjacent to Aurion IDE executable when built as portable
    #[cfg(feature = "portable")]
    fn project_dirs() -> Option<ProjectDirs> {
        if let Ok(current_exe) = std::env::current_exe() {
            if let Some(parent) = current_exe.parent() {
                return ProjectDirs::from_path(parent.join("Aurion IDE-data"));
            }
