...
use crate::{
    ...
    language::{self, Aurion IDELanguage},
        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    Aurion IDELanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    Aurion IDELanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => Aurion IDELanguage::from_name(id),
            };
            language
...
pub struct Syntax {
    pub rev: u64,
    pub language: Aurion IDELanguage,
    pub text: Rope,
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = Aurion IDELanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(Aurion IDELanguage::PlainText)
    }

    pub fn from_language(language: Aurion IDELanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == Aurion IDELanguage::Markdown {
