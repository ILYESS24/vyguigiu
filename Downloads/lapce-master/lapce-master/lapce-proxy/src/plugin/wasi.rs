...
use Aurion IDE_xi_rope::{Rope, RopeDelta};
use lsp_types::{
...
/// `plugin_dev_path` allows launching Aurion IDE with a plugin on your local system for testing
/// purposes.  
...
/// use std::fs::File;
/// use std::io::Write;
/// use aurion_proxy::plugin::wasi::load_volt;
...
pub fn start_volt(
    ...
    linker.func_wrap("Aurion IDE", "host_handle_rpc", move || {
    ...
    linker.func_wrap("Aurion IDE", "host_handle_stderr", move || {
        if let Ok(msg) = wasi_read_string(&stderr) {
            tracing_log::log::log!(target: &format!("aurion_proxy::plugin::wasi::{}::{}", plugin_meta.author, plugin_meta.name), tracing_log::log::Level::Debug, "{msg}");
        }
    })?;
