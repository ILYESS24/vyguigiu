...
$proxy = (Join-Path $directory 'aurion-proxy.exe')

$aurionProcesses = (Get-Process -Name 'aurion-proxy' -EA SilentlyContinue).Count
if ($aurionProcesses -ne 0) {
...
$url = "https://github.com/aurion-ide/aurion-ide/releases/download/${version}/aurion-proxy-windows-${arch}.gz"
$gzip = Join-Path "${env:TMP}" "aurion-proxy-windows-${arch}.gz"
...
Write-Host 'aurion-proxy installed'
