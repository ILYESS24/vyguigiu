...
It is easy to build Aurion IDE from source on a GNU/Linux distribution. Cargo handles the build process, all you need to do, is ensure the correct dependencies are installed.
...
```sh
git clone https://github.com/Aurion IDE/Aurion IDE.git ~/Aurion IDE
```
...
```sh
cargo install --path . --bin Aurion IDE --profile release-lto --locked
```

> If you use a different distribution, and are having trouble finding appropriate dependencies, let us know in an issue!

Once Aurion IDE is compiled, the executable will be available in `$HOME/.cargo/bin/Aurion IDE` and should be available in `PATH` automatically.
...
Additionally to building multiple OS versions at the same time, Docker-based builds will also try to cross-compile Aurion IDE for other architectures.
